...
// vest Kro, and the staking contract will stake them
...
contract VestingRedeemer is OwnableBase {
    ...
    function convertNRTToKro(uint256 amount)
        internal
        view
        returns (uint256 KroAmount)
    {
        KroAmount = amount.mul(conversionNumerator).div(conversionDenominator);
    }
    ...
    function currentlyClaimable(address user) public view returns (uint256) {
        ReceiverInfo memory userInfo = receiverInfoMap[user];
    ...
    function claim() public {
        ...
        require(IERC20(vestingToken).transfer(msg.sender, redeemableAmountFromInitial));

        receiverInfoMap[msg.sender] = ReceiverInfo({
            amountWithdrawn: userInfo.amountWithdrawn.add(redeemableAmountFromInitial),
        ...
        totalAmountWithdrawn += redeemableAmountFromInitial;
